...
const ShoppinApp = () => {
  ...
  const removeItem = (index) => {
    ...
    for (let i = 0; i < itemsList.length; i++) {
      if (i !== index) {listAfterRemove.push(itemsList[i])  }
    }
  ...
  return (
    <div className={styles.ShoppinApp}>
      <h3 className={styles.title}>Shoping list App</h3>
      <input
      ...
      <button className={styles.btnAddEdit}
        onClick={(input) => (buttonStatus) ? addNewItem(inputValue) : updateItem(inputValue)}>{buttonStatus ? 'Add' : 'Update'}</button>
      <button className={styles.clearBtn} style={(clearBtnStatus) ? { display: "inline-block" } : { display: 'none' }}
        onClick={() => clearAll()}>Clear</button>
      <p style={{ color: "red", backgroundColor: "black", fontSize: "20px", fontWeight: "bold" }}>{inputError}</p>
      
      <div className={styles.itemsComponent} style={(itemsList.length > 0) ? { display: "block" } : { display: "none" }}>
        {itemsList.map((item, index) => (
          <div key={index}>
            <div className={styles.itemComponent}> <span>{`${index + 1}. ${item}`}</span>
              <div className={styles.itemBtns}><button className={styles.editBtn} onClick={() => { editItem(item) }}>Edit</button>
                <button className={styles.deleteBtn} onClick={() => { removeItem(index) }}>Remove</button>
              </div>
            </div>
          </div>
        ))}
